...
export default function SchedulePanel({ 
  ...
  targetBatch, setTargetBatch, targetGroup, setTargetGroup,
  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fadeIn">
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="bg-slate-900/40 border border-rose-500/30 p-6 rounded-2xl relative">
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              ...
              <select value={targetGroup} onChange={(e) => setTargetGroup(e.target.value)} className="bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs text-slate-200"><option value="A">Group A</option><option value="B">Group B</option><option value="C">Group C</option></select>
            </div>
      ...
      <div className={`bg-slate-900/40 border border-slate-800 p-6 rounded-2xl ${isAdmin ? "md:col-span-2" : "md:col-span-3 max-w-3xl mx-auto w-full"}`}>
        <div className="mb-6">
          <h3 className="text-lg font-bold text-slate-200">{isAdmin ? `Viewing Cohort: ${currentTargetId}` : "Your Official Timetable"}</h3>
          <p className="text-sm text-slate-400">{isAdmin ? "Use the dropdowns to view and manage other cohort schedules." : "This schedule is centrally managed by your college administration."}</p>
        </div>
          ...
          {daysOfWeek.filter(d => d !== "Sunday").map(day => (
            <div key={day} className="border-b border-slate-800 pb-3">
              <div className="flex justify-between mb-2">
                ...
                {isAdmin && timetable[day]?.length > 0 && <button onClick={() => handleClearDaySchedule(day)} className="text-xs text-rose-400">Clear</button>}
              </div>
